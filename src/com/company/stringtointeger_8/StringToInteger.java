package com.company.stringtointeger_8;

public class StringToInteger {
    private static int myAtoi(String str) {
        //去除掉前后的空格
        String strr = str.trim();
        //存储最终过滤出来的字符串
        String strrr = null;
        //字符串不为空时并且字符串不全是空白字符串时才转换
        if (!strr.isEmpty()) {
            char first = strr.charAt(0);
            //判断字符串中的第一个非空格字符是不是一个有效整数字符
            if (first >= '0' && first <= '9' || first == '+' || first == '-') {
                strrr = strr.substring(0, 1); // 把第一位放进去(只能是数字、正负号)
                //这时候循环只要数字，因为正负号只能出现在第一位
                for (int i = 1; i < strr.length(); i++) {
                    if (strr.charAt(i) >= '0' && strr.charAt(i) <= '9') {
                        strrr = strr.substring(0, i + 1);
                    }
                    //这是遇到不符合要求的字符，直接忽略剩余元素
                    else break;
                }
            }
        }
        //判断最终字符串是否为空或只有一个正负号
        if (strrr == null || strrr.equals("+") || strrr.equals("-"))
            //此时strrr是String对象，如果使用==比较则比较的时内存地址
            return 0;
        //最终转换成的数字
        int num;
        //使用异常机制打印结果
        try {
            num = Integer.parseInt(strrr);
        } catch (Exception e) {
            if (strrr.charAt(0) == '-')
                return Integer.MIN_VALUE;
            return Integer.MAX_VALUE;
        }
        return num;
    }

    private static int myNewAtoi(String str) {
        String middlerStr = str.trim();
        int i = 0, sign = 1;
        if (i < middlerStr.length()) {
            char csign = middlerStr.charAt(i);
            if (csign == '-') {
                sign = -1;
                i++;
            } else if (csign == '+') {
                sign = 1;
                i++;
            } else if (csign < '0' || csign > '9') {
                return 0;
            }
        }

        long number = 0L;
        for (; i < middlerStr.length(); i++) {
            char c = middlerStr.charAt(i);
            if (c < '0' || c > '9') {
                break;
            }
            number = number * 10 + (c - '0');

            if (number * sign > Integer.MAX_VALUE) {
                return Integer.MAX_VALUE;
            }
            if (number * sign < Integer.MIN_VALUE) {
                return Integer.MIN_VALUE;
            }
        }
        return (int) number * sign;
    }

    public static void main(String[] args) {
        int num = myAtoi("20000000000000000000");
        int num_1 = myNewAtoi("20000000000000000000");
        int num_2 = myNewAtoi(" -42");
        System.out.println(num);
        System.out.println(num_1);
        System.out.println(num_2);
    }
}
